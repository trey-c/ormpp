...
public:
    ...
    constexpr int update(const std::vector<T> &v, Args &&... args)
    {
        ...
        for (auto &t : v) {
            auto condition =
                get_condition(t, key, std::forward<Args...>(args)...);
    ...
    {
        auto sql =
            generate_delete_sql<T>(std::forward<Args>(where_conditon)...);
        res_ = PQexec(con_, sql.data());
        if (PQresultStatus(res_) != PGRES_COMMAND_OK) {
            PQclear(res_);
            return false;
...
private:
            ...
            iguana::for_each(
                ...
                [&sql, &i, &has_add_field, field_name, type_name_arr, name,
                 this](auto item, auto I) {
                    if constexpr (std::is_same_v<decltype(item),
                                                 ormpp_not_null>) {
                        if (item.fields.find(field_name.data()) ==
                            item.fields.end())
                            return;
                    } else {
    ...
    {
        res_ = PQprepare(con_, "", sql.data(), (int)iguana::get_value<T>(),
                         nullptr);
        if (PQresultStatus(res_) != PGRES_COMMAND_OK) {
            std::cout << PQresultErrorMessage(res_) << std::endl;
            PQclear(res_);
                              ...
                              Args &&... args)
    {
        ...
        iguana::for_each(
            t, [&t, &param_values, &auto_key, this](auto item, auto i) {
    ...
    template <typename T>
    constexpr void
    set_param_values(std::vector<std::vector<char>> &param_values, T &&value)
    {
